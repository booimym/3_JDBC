...
public class BoardService {
	...
	public int insertBoard(Board board) throws Exception  {
		...
		//게시글 작성 시, 게시글 상세조회도 하게 해주고 싶어!
		//게시글 번호 생성 DAO를 호출하기
		//왜? 동시에 여러 사람이 게시글을 등록하면
		//시퀀스가 한번에 증가하여 CURRVAL 구문을 이용하면 문제가 발생한다.
		
		//-> 게시글 등록 서비스를 호출한 순서대로 
		//	미리 게시글 번호를 생성해서 얻어온 다음 이를 이용해 INSERT를 하겠다.
		...
		if (result>0 ) { commit (conn);
			...
			result = boardNo;
			//INSERT 성공시 생성된 게시글 번호(boardNo)를 결과로 반환시키겠다.
